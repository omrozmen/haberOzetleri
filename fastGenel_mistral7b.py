...
Komutu herhangi bir argüman vermeden `python fastGenel_mistral7b.py` şeklinde çalıştırabilirsiniz.
"""
...
def get_run_config() -> Dict:
    ...
    return {
        ...
        "output_dir": Path("haberOzetleri_mistral"),
        "model": DEFAULT_MODEL,
...
def _build_prompt(body: str, title: str | None) -> str:
    talimat = (
        ...
        "Anlam bütünlüğünü koru; olayların ardışık bağını bozmadan tutarlı bir anlatım kur. "
...
def _merge_new_entries_first(output_data: Dict[str, Dict], new_items: List[Dict]) -> Dict[str, Dict]:
    """
    Yeni oluşturulan özetleri JSON'da üstte gösterecek şekilde yeniden sıralar.
    """
    if not new_items:
        return output_data
    ...
    for item in new_items:
        title = item["title"]
        if title in output_data and title not in ordered:
            ordered[title] = output_data[title]

    for title, payload in output_data.items():
        if title not in ordered:
            ordered[title] = payload
...
def _process_single_file(input_path: Path, output_path: Path, cfg: Dict) -> int:
    ...
    ordered_output = _merge_new_entries_first(output_data, summarized_targets)
