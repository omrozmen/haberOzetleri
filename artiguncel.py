...
UPDATED_CONTENT_DIR = "HaberIcerikleriGuncel"

os.makedirs(TITLE_DIR, exist_ok=True)
os.makedirs(CONTENT_DIR, exist_ok=True)
os.makedirs(UPDATED_CONTENT_DIR, exist_ok=True)
...
def _updated_content_file(name):
    return os.path.join(UPDATED_CONTENT_DIR, f"{_slugify(name)}.json")
...
sites_config = [
    {
        ...
        "updated_content_file": _updated_content_file("Ensonhaber"),
    ...
    },
    {
        ...
        "updated_content_file": _updated_content_file("Malatyahaber"),
        ...
        "updated_content_file": _updated_content_file("Haberler"),
        "tags": ["p", "h1", "h2","blockquote"]
    },
        {
        "name": "internethaber",
        ...
        "updated_content_file": _updated_content_file("internethaber"),
    ...
    },
     {
        ...
        "updated_content_file": _updated_content_file("İnternetSpor"),
        "tags": ["p", "h1", "h2","blockquote"]
    }
]
...
def build_updated_content_blocks(combined_content):
    """
    Diğer etiketleri aynen korurken, ardışık <p> bloklarını tek bir
    kayıt altında satır boşluklarıyla birleştirir.
    """
    merged_blocks = []
    paragraph_buffer = []
    ...
    for block in combined_content:
        ...
        if tag == "p":
            ...
            flush_paragraphs()
            merged_blocks.append({"tag": tag, "text": text})
...
def scrape_contents_only(config):
    ...
    try:
        ...
        existing_contents = load_existing_contents(config["content_file"])
        existing_updated_contents = load_existing_contents(config["updated_content_file"])
        
        # Eksik içerikleri topla
        new_contents = {}
        new_updated_contents = {}
        for title in existing_titles.values():
            needs_standard = title not in existing_contents
            needs_updated = title not in existing_updated_contents
            if title in title_url_map and (needs_standard or needs_updated):
                ...
                if combined_content:
                    ...
                    if needs_updated:
                        updated_blocks = build_updated_content_blocks(combined_content)
                        if updated_blocks:
                            new_updated_contents[title] = {
                                "url": url,
        ...
        if new_updated_contents:
            save_to_json(config["updated_content_file"], new_updated_contents)
            logging.info(f"{config['name']} için {len(new_updated_contents)} yeni içerik '{config['updated_content_file']}' dosyasına kaydedildi.")
        if not new_contents and not new_updated_contents:
...
def scrape_and_save_combined_text(config):
    ...
    try:
        ...
        if new_updated_contents:
            save_to_json(config["updated_content_file"], new_updated_contents)
            logging.info(f"{config['name']} için yeni içerikler '{config['updated_content_file']}' dosyasına kaydedildi.")
        if not new_contents and not new_updated_contents:
            logging.info(f"{config['name']} için yeni içerik bulunamadı.")
...
def check_and_generate_missing_files():
    ...
    for site_config in sites_config:
        ...
        updated_content_file = site_config["updated_content_file"]
        
        title_exists = os.path.exists(title_file)
        content_exists = os.path.exists(content_file)
        updated_exists = os.path.exists(updated_content_file)
        ...
        logging.info(f"  Güncel içerik dosyası ({os.path.basename(updated_content_file)}): {'VAR' if updated_exists else 'EKSİK'}")
        ...
        existing_updated_contents = load_existing_contents(updated_content_file)
        missing_titles = [
            title for title in existing_titles.values()
            if title not in existing_contents or title not in existing_updated_contents
        ]
