...
def _build_prompt(body: str, title: str | None) -> str:
    talimat = (
        ...
        "Metni zihninde bütün halde analiz ederek ana tema, kritik aktörler, önemli veriler, zaman/mekân ve sonuç ilişkisini çıkar. "
        "Sebep-sonuç zincirini bozmayacak biçimde en fazla dört cümleden oluşan, tercihen 3-4 cümlelik tek paragraf özet yaz; her cümle bir öncekini tamamlayarak anlam bütünlüğünü korusun. "
        "Çıktı dili kesinlikle Türkçe olsun; gereksiz tekrar, başlık formatı, madde işareti, liste, İngilizce ifade ve öz referans kullanma. "
        "Kişi/kurum adları, kritik olaylar, sayısal veriler ve sonuç cümlesi mutlaka yer alsın; metnin tamamını kapsayan bağlamsal bir anlatı oluştur. "
        "Cevabı doğrudan özet paragrafıyla ver ve tırnak işareti kullanma."
    )
    if title:
        return f"{talimat}\n\nBaşlık: {title.strip()}\nMetin:\n{body.strip()}\n\nÖzet:"
    return f"{talimat}\n\nMetin:\n{body.strip()}\n\nÖzet:"
